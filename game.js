// Car Arena Combat 2D - Consolidated working build
// Includes: CONFIG/CARS, controls, physics, HUD, victory overlay, fullscreen sizing

// ===== 1) CONFIG =====
const CONFIG = { winStars:5, roundTime:180, gravity:0.6, friction:0.98, groundFriction:0.88, jumpImpulse:14, restitution:0.6, launchSlopeThreshold:0.25, launchSpeedThreshold:3.0, launchBoost:0.45 };

// ===== 2) CARS (same size) =====
const CARS = { basic:{ speed:4.4, size:64, color:'#4FC3F7' }, truck:{ speed:4.4, size:64, color:'#F44336' } };
...
function resizeCanvas(){ const dpr = window.devicePixelRatio||1; const rect = document.getElementById('gameContainer'); canvas.width = rect.clientWidth*dpr; canvas.height = rect.clientHeight*dpr; ctx.setTransform(dpr,0,0,dpr,0,0); W = rect.clientWidth; H = rect.clientHeight; }
...
const PLATFORMS = [ ()=>({ x: W*0.18, y: H-240, w: 160, h: 18 }), ()=>({ x: W*0.55, y: H-220, w: 200, h: 18 }), ()=>({ x: W*0.78, y: H-260, w: 140, h: 18 }) ];
...
function initGame(){ hideVictory(); game={ timeLeft:CONFIG.roundTime, running:false, winner:null, lastTime:0, players:[ createPlayer(W*0.2,heightAt(W*0.2)-60,'basic','J1'), createPlayer(W*0.8,heightAt(W*0.8)-60,'truck','J2') ], lastPointText:'', lastPointTime:0, platforms: currentPlatforms() }; updateUI(); }
...
function resolveHeadKills(){ const [p1,p2]=game.players; let w=null, reason=''; if(headHit(p1,p2)){ w=0; reason='Head Hit'; } if(headHit(p2,p1)){ w=1; reason='Head Hit'; } if(w!==null){ addPoint(w,reason); p1.x=W*0.2; p1.y=heightAt(W*0.2)-60; p1.vx=p1.vy=0; p2.x=W*0.8; p2.y=heightAt(W*0.8)-60; p2.vx=p2.vy=0; } }
