// Car Arena Combat 2D - Drive Ahead! Style (consolidated)
// Ce fichier a été consolidé pour corriger "CONFIG is not defined" et unifier la physique.
...
const CONFIG = {
  ...
  jumpImpulse: 14,
  restitution: 0.6,
  launchSlopeThreshold: 0.25, // pente minimale pour décoller
  launchSpeedThreshold: 3.0,   // vitesse minimale pour décoller
  launchBoost: 0.45            // intensité du décollage liée à la vitesse
};
...
function updatePlayer(player, leftKey, rightKey, jumpKey){
  ...
  if(player.y >= groundY - player.car.size/2){
    ...
    if(player.vy>0) player.vy *= -0.2; // petit rebond
    player.onGround = true;
    player.vx *= CONFIG.groundFriction;
    // décollage selon pente et vitesse
  ...
  for(const pf of PLATFORMS){
    ...
    if(player.x>left && player.x<right){
      if(player.vy>0 && player.y + r >= top && player.y - r < top){
        player.y = top - r; player.vy = 0; player.onGround = true;
      }
    }
  }
...
function separatePlayers(p1, p2){
  ...
  if(dist < minDist){
    ...
    if(sepVel < 0){
      ...
      p1.vx -= j*nx; p1.vy -= j*ny; p2.vx += j*nx; p2.vy += j*ny;
    }
  }
}
...
function headHit(attacker, defender){
  ...
  const dx = hx - defender.x, dy = hy - defender.y;
...
function resolveHeadKills(){
  ...
  if(headHit(p1,p2)) winnerIndex = 1;
  if(headHit(p2,p1)) winnerIndex = 2;
  if(winnerIndex){
    const idx = winnerIndex-1;
    game.players[idx].stars++;
    if(game.players[idx].stars>=CONFIG.winStars){
...
function render(){
  ...
  grad.addColorStop(0,'#263238'); grad.addColorStop(1,'#1b5e20');
  ctx.fillStyle = grad; ctx.fillRect(0,0,W,H);

  // Sol sinusoïdal
  ctx.strokeStyle = '#6d4c41'; ctx.lineWidth = 6; ctx.beginPath();
  for(let x=0;x<=W;x+=10){ const y = heightAt(x); if(x===0) ctx.moveTo(x,y); else ctx.lineTo(x,y); }
  ctx.stroke();
  ctx.fillStyle = '#79554844'; ctx.lineTo(W,H); ctx.lineTo(0,H); ctx.closePath(); ctx.fill();
  ...
  for(const p of game.players){
    ...
    ctx.arc( p.car.size/3, p.car.size/4, p.car.size/8, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle = '#ff5252'; ctx.beginPath(); ctx.arc(0,-p.car.size*0.35, p.car.size*0.18, 0, Math.PI*2); ctx.fill();
    ctx.restore();
  }
}
...
function gameLoop(ts){
  ...
  game.lastTime = ts; update(dt); render(); updateUI();
